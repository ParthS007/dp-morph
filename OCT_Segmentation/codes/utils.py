    ...
    for j in range(batch_size):
        ...
        if num_class > 1:
            ...
            image_pred = image_pred.reshape(
                (image_pred.shape[0] * image_pred.shape[1], -1)
            )
            image_true = image_true.reshape(
                (image_true.shape[0] * image_true.shape[1], -1)
            )
            ...
            for i in range(num_class):
                ...
                union = (
                    GT.sum() + Pred.sum()
                )  # + 0.0001 # this extra value makes the calculation incorrect
                # print(f"union: {union}")
                if union == 0:  # prevent division by zero
                    dice = torch.tensor(
                        1.0, device=Pred.device
                    )  # perfect match when nothing exists
                else:
                    dice = 2 * inter / union
                # print(f"dice: {dice}")
...
def compute_pa(ground_truth, prediction):
    ...
    try:
        ...
        for i in range(9):
            ...
            if mask1.sum() != 0:
                ret[i] = float(
                    ((mask1 * (ground_truth == prediction)).sum()) / (mask1.sum())
                )
            else:
                ret[i] = float("nan")
    except Exception as e:
...
def categorical_mae(y_pred, y_true):
    ...
    for i in range(y_true.size(1)):  # iterating over each class
        ...
        if class_mask.any():
            ...
            class_mae.append(
                torch.tensor(0.0)
            )  # Avoid division by zero if no elements of this class
...
def MAE_New(
    y, p, n_classes, classes=None
):  # classes can be for example : [1,2,3,4,5,6,7,...]
    p = F.softmax(p, dim=1)
    # print(y.shape)
    # print(p.shape)
    B, C, H, W = y.shape
...
def MAE_secondversion(ground_truth, prediction):
    ...
    for class_idx in range(num_classes):
        gt_class = ground_truth[
            :, class_idx, ...
        ]  # Select the current class for all batches
